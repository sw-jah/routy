...
export default function FoldersPage() {
  ...
  const [isInitialLoading, setIsInitialLoading] = useState<boolean>(true); // 💡 초기 방 목록 조회 상태 플래그
  ...
  const fetchMyRooms = useCallback(
    async (user: string, targetCode?: string) => {
      try {
        ...
        setIsInitialLoading(false); // 💡 조회가 끝나면 초기 로딩 해제
      }
    },
    [fetchRoomCourses]
  );
  ...
  return (
    <main className="max-w-md mx-auto min-h-screen px-5 pt-6 pb-12 flex flex-col gap-4 bg-[#FAF7F2] text-[#2D241E]">
      ...
      {/* 💡 1. 최초 방 목록 조회 중일 때 로딩 뷰 표시 */}
      {isInitialLoading ? (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4 animate-in fade-in duration-200">
          <div className="w-10 h-10 border-4 border-[#EADFCF] border-t-[#C25E3E] rounded-full animate-spin" />
          <div className="text-center">
            <p className="font-title text-base text-[#2D241E]">코스 보관함을 불러오는 중입니다...</p>
            <p className="font-body text-xs text-[#8C7A6B] mt-1">참여 중인 약속 방을 찾고 있어요 📁</p>
          </div>
        </div>
      ) : (
        <>
